...
async function loadMessages() {
  ...
  try {
    const data = await get(API_ENDPOINTS.SESSION_MESSAGES(currentSessionId.value))
...
<template>
  <div class="app-container relative min-h-screen bg-background overflow-hidden">
    <GridBackground class="opacity-30" />
      ...
      <header class="header glass-effect">
        <div class="header-left">
          <button
            ...
            class="menu-btn hover-lift"
          >
            <svg class="icon" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16" />
            </svg>
          </button>
          <h1 class="title gradient-text">MyClaw</h1>
        </div>
